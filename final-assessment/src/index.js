...
function renderContact(contact) {
  const {id, picture, name, email, phone, website, address:{street}, address: {suite}, address: {city}, address: {zipcode}, company} = contact;
...
function filterByCity(city) {
  const filteredByCity = contacts.filter((contact) => {
    const homeCity = contact.address.city.toLowerCase();
...
function filterHandler() {
    const city = document.querySelector("#filterOptions").value;
    if(city === "All"){
      render(contacts);
    } else{
...
*/
function main() {
  //create city search event listener and call handler
  const citySelected = document.querySelector("#filterOptions");
  citySelected.addEventListener("change", filterHandler);

  //create event listener for clicks
  const main = document.querySelector("main");

  //call the deleteButtonHandler
